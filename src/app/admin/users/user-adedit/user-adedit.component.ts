...
@Component({
  selector: 'app-user-adedit',
  templateUrl: './user-adedit.component.html',
  styleUrls: ['./user-adedit.component.scss']
})
export class UserAdeditComponent implements OnInit {
  ...
  async submit(){
    ...
    // return;
    // --create
    const res = await this.userService.register(formInput);
    console.log('res-', res);
    if(res.status === 200){
      let _html=`
              <div class="c-green">
                <div class="material-icons">task_alt</div>
                <h1>User Created Success!</h1>
              </div>`;
  ...
  async update(){
    ...
    if(res.status === 200){
      let _html=`
              <div class="c-green">
                <div class="material-icons">task_alt</div>
                <h1>User Update Success!</h1>
              </div>`;
      this.openDialog(_html);
      this.router.navigate([`/admin/user-adedit/${formInput.id}`]);
    }else{
      this.Errors.status = true;
      this.Errors.msg = res.message;
    }
